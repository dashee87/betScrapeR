...
partialMatch=function(names1,names2){
  names1.split <- sapply(names1,FUN=function(x)strsplit(x," "),USE.NAMES = FALSE)
  names2.split <- sapply(names2,FUN=function(x)strsplit(x," "),USE.NAMES = FALSE)
  if(length(names2)==1){
  output <- as.data.frame(cbind(c(1,1),sapply(names1.split, FUN = function(x) {sapply(names2.split, FUN = function(y) {sum(x %in% y)}, USE.NAMES = FALSE) }, USE.NAMES = FALSE)))
  }else{output <- as.data.frame(t(sapply(as.data.frame(sapply(names1.split,FUN=function(x){sapply(names2.split,FUN=function(y){sum(x %in% y)},USE.NAMES = FALSE)},USE.NAMES = FALSE)),FUN=function(z){c(which.max(z),max(z))})))}
